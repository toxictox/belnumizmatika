	...
	switch ($woof_settings['tax_type'][$tax_slug]) {
		...
		if ($WOOF->settings['show_title_label'][$tax_slug]) {
		    ?> <div class="options-item">
                        <p class="options-title"><?php echo $taxonomies_info[$tax_slug]->labels->singular_name; ?></p>
			    <?php
			}
		...
		<?php if ($price_filter == 3): ?>
		    ...
		    <?php if (isset($WOOF->settings['by_price']['title_text']) AND ! empty($WOOF->settings['by_price']['title_text'])): ?>
				<p class="options-title"><?php _e('Цена', 'woocommerce-products-filter');?>:</p>
		    <?php endif; ?>
...
?>
	...
	?>
			...
			<?php if (!$autosubmit OR $ajax_redraw): ?>
			    ...
			    ?>
				<button id="searchgo" style="float: left;" class="options-submit mobile-options-submit"><?php echo $woof_filter_btn_txt ?></button>
			<?php endif; ?>
