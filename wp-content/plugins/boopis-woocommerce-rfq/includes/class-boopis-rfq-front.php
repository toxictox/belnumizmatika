...
if ( ! class_exists( 'BOOPIS_RFQ_Front' ) ) {
	...
	class BOOPIS_RFQ_Front {
		...
		public function boopis_add_to_quote_text( $link ) {
			...
			} elseif ( get_option('boopis_rfq_quote_trigger') == false && $product->get_type() == "simple_rental" ) {
				$link = __( 'Inquire', 'boopis-woocommerce-rfq' );
			}
		...
		public function boopis_add_to_quote_url( $link ) {
			...
			if ( get_option('boopis_rfq_quote_trigger') == true && $this->searchForTag(get_option('boopis_rfq_tag_trigger_value'), get_the_terms($product->id, 'product_tag')) || ( get_option('boopis_rfq_quote_trigger') == false && $product->get_type() == "simple_rental" ) ) {
