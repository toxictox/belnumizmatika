...
<?php do_action('woocommerce_before_bid_form'); ?>
		<form method="post" enctype='multipart/form-data' data-product_id="<?php echo $product_id; ?>">
			...
			<?php if($product->get_auction_type() == 'reverse' ) : ?>
				<div class="quantity buttons_added">
					...
					<input type="number" name="bid_value" data-auction-id="<?php echo esc_attr( $product_id ); ?>"  <?php if ($product->get_auction_sealed() != 'yes'){ ?> value="<?php echo $product->bid_value() ?>" max="<?php echo $product->bid_value()  ?>"  <?php } ?> step="any" size="<?php echo strlen($product->get_curent_bid())+2 ?>" title="bid"  class="auc-price">
			...
			<?php else : ?>	
					...
					<input type="number" name="bid_value" data-auction-id="<?php echo esc_attr( $product_id ); ?>" <?php if ($product->get_auction_sealed() != 'yes'){ ?>  value="<?php echo $product->bid_value()  ?>" min="<?php echo $product->bid_value()  ?>" <?php } ?>  step="any" size="<?php echo strlen($product->get_curent_bid())+2 ?>" title="bid"  class="auc-price">
					
				
		 	<button type="submit" class="single-product_buy"><?php echo apply_filters('bid_text', __( 'Bid', 'wc_simple_auctions' ), $product); ?></button>
		 	<?php endif; ?>
